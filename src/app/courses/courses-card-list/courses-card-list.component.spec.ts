...
describe('CoursesCardListComponent (Angular Testing Library)', () => {
    ...
    it('should display the first course', async () => {
        ...
        const { container } = await render(CoursesCardListComponent, {
            imports: [CoursesModule],
            componentInputs: {
                courses: courses
            }
        ...
        const firstCourseCard = container.querySelector('.course-card');
        expect(firstCourseCard).toBeTruthy();

        const cardScope = within(firstCourseCard as HTMLElement);
        const imageElement = cardScope.getByRole('img');
